...
class Client(BaseClient):
    ...
    def update_account_name(self, id, name):
        """Make a call to PUT /api/1/accounts/{id}/name.
    ...
    def validate(
        ...
        """Make a call to POST /api/1/address/validate.
        ...
        :type country: str
        :param date_of_birth: DateOfBirth is the date of birth of the (non-institutional) beneficial owner of the
                              address in the form "YYYY-MM-DD"
        ...
        :param has_destination_tag: Optional boolean flag indicating that a XRP destination tag is provided (even if
                                    zero).
        :type has_destination_tag: bool
        :param institution_name: InstitutionName is the name of the beneficial owner if is it is a legal entities
                                 address
        :type institution_name: str
        :param is_legal_entity: IsLegalEntity indicates if the address is for a legal entity and not a private
                                beneficiary. If this field is true then the fields BeneficiaryName, Nationality &
                                DateOfBirth should be empty but the fields InstitutionName and Country should be
                                populated. If this field is false and IsSelfSend is false (or empty) then the field
                                InstitutionName should be empty but the fields BeneficiaryName, Nationality &
                                DateOfBirth and Country should be populated.
        :type is_legal_entity: bool
        :param is_private_wallet: IsPrivateWallet indicates if the address is for private wallet and not held at an
                                  exchange.
        ...
        :param memo: Optional memo string used to provide account information for ATOM, etc. where it holds "account"
                     information for a generic address.
        :type memo: str
        :param nationality: Nationality ISO 3166-1 country code of the nationality of the (non-institutional) beneficial
                            owner of the address
        :type nationality: str
        :param physical_address: PhysicalAddress is the legal physical address of the beneficial owner of the crypto
                                 address
